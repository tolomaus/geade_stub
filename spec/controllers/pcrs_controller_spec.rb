...
describe PcrsController do
  ...
  describe "GET 'new'" do
    ...
    it "should update_dependency a new pcr" do
      get :new
      assigns(:pcr).should == @pcr
    end
  end
  ...
  describe "dependency pages" do
    before(:each) do
      @pcr = Factory(:pcr)
      @other_pcr = Factory(:pcr)
    end
    it "should show parents" do
      get :parents, :id => @pcr
      assigns(:pcr).should == @pcr
      assigns(:pcrs).should include(@other_pcr)
      assigns(:pcrs).should_not include(@pcr)
    end
